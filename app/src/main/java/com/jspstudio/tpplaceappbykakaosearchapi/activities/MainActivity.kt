...
import com.jspstudio.tpplaceappbykakaosearchapi.model.PlaceMeta
...
class MainActivity : AppCompatActivity() {
    ...
    // 카카오의 지역검색(로컬) API - 카카오개발자 사이트 참고
    // 1. 검색 장소명
    var searchQuery:String = "화장실" // 앱 초기 키워드 - 내 주변 개방화장실
    // 2. 현재 내 위치정보 객체 (위도,경도 정보를 멤버로 보유)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 소프트키보드의 검색버튼이 클릭됐을 때
        binding.etSearch.setOnEditorActionListener { textView, i, keyEvent ->
            searchQuery= binding.etSearch.text.toString()
            searchPlaces()

            // return 값이 있는 메소드
            // 소프트키보드의 액션버튼이 클릭되었을 때 여기서 모든 액션을 소모하지 않겠다는 뜻으로 false 리턴
        ...
        if (checkSelfPermission(permissions[0]) == PackageManager.PERMISSION_DENIED){
            // 허가되어 있지 않으므로 권한을 요청하는 다이얼로그 보이기
            requestPermissions(permissions, 10)
        }else{
            // 허가되어 있으므로 곧바로 내 위치값을 요구하기
            requestMyLocation()
        }

    } // onCreate method

    // 퍼미션요청 다이얼로그의 [허가/거부]선택이 되었을 때 자동 발동하는 콜백메소드
    override fun onRequestPermissionsResult(
        ...
        else Toast.makeText(this, "내 위치정보를 제공하지 않았기에 검색기능을 사용할 수 없습니다", Toast.LENGTH_SHORT).show()
    }
    ...
    // 위치정보 갱신될 때마다 발동하는 Callback 객체 생성
...
//                AlertDialog.Builder(this@MainActivity).setMessage("${meta?.total_count} \n ${documents?.get(0)?.place_name}").show()
    ...
    private fun setChoiceButtonsListener(){
        ...
        binding.layoutChoice.choicePharmecy.setOnClickListener{ clickChoice(it) }
    ...
    private fun clickChoice(view: View){
        ...
        when( view.id ){
            ...
            R.id.choice_pharmecy->searchQuery="약국"
